...
def _registry(encoding):  # pragma: no cover
    if encoding == 'latex':
...
def _tokenize(tex):  # pragma: no cover
    ...
    while 1:
        if pos > start:
            yield tex[start:pos]
            if (
                tex[start] == '\\' and not (tex[pos - 1].isdigit() and tex[start + 1].isalpha())
            ):
...
class _unlatex(object):  # pragma: no cover
    ...
    def __next__(self):
        ...
        if (
            self.lastoutput[0] == '\\' and self.lastoutput[-1].isalpha() and nextoutput[0].isalpha()
        ):
