...
class Database(object):
    ...
    def trickle(self, bibfiles):
        ...
        with self.connect() as conn:
            for file_pk, filename in conn.execute(select_files).fetchall():
                ...
                print(f'{changed:d} changed {added:d} added in {b.id}')
                b.save(entries)
    ...
    def __iter__(self, *, chunksize: int = 100):
        ...
        with self.connect() as conn:
            for first, last in Entry.windowed(conn, colname='id', chunksize=chunksize):
    ...
    def _print_group(conn, group, *, out=print):
        for row in group:
            out(row)
        for row in group:
            out('\t%r, %r, %r, %r' % Value.hashfields(conn,
...
class Entry(Model):
    ...
    def stats(cls, *, conn, out=log.info):
        ...
        out('\n'.join(f'{r.filename} {r.n:d}' for r in conn.execute(select_n)))
